...
export default function LoginPage() {
    ...
    const applyPrivilegedAccess = async (email?: string | null) => {
        if (!email) return;
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (isLogin) {
                ...
                if (signInError) {
                    setNotice(signInError.message);
                    return;
                }

                await applyPrivilegedAccess(data.user?.email);
            ...
            setNotice("회원가입 완료! 이메일로 보낸 인증 링크를 눌러 계정을 활성화한 뒤 로그인해 주세요.");
            setIsLogin(true);
        } finally {
            setLoading(false);
        }
                ...
                <div className="glass rounded-2xl p-8 space-y-6">
                    ...
                    {error && (
                        <div className="rounded-xl border border-red-300/50 bg-red-50 px-3 py-2 text-xs text-red-700">
                            로그인 오류: {safeDecode(error)}
                        </div>
                    )}

                    {notice && (
                        <div className="rounded-xl border border-amber-300/50 bg-amber-50 px-3 py-2 text-xs text-amber-700">
                            {notice}
                        </div>
                    )}
                    ...
                    <div className="space-y-3">
                        <button
                            ...
                            disabled={loading}
                            className="w-full py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-60"
                        >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        {!isLogin && (
                            <div>
                                <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">이름</label>
                                <input
                                    name="name"
                                    type="text"
                                    placeholder="홍길동"
                        ...
                        <button type="submit" disabled={loading} className="w-full btn-primary justify-center !py-3 mt-2 disabled:opacity-60">
